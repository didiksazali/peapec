...
$qry = mysql_query("SELECT *
							...
							INNER JOIN tbl_ukm ON tbl_barang.id_ukm = tbl_ukm.id_ukm where tbl_barang.id_ukm = '$idUKM' LIMIT $offset, $dataPerhalaman ");
...
$qry2 = mysql_query("SELECT COUNT(*) as jumlah
							FROM tbl_kategori
							INNER JOIN tbl_barang ON tbl_kategori.id_kategori = tbl_barang.id_kategori
							INNER JOIN tbl_ukm ON tbl_barang.id_ukm = tbl_ukm.id_ukm where tbl_barang.id_ukm = '$idUKM'");
...
if ($nohalaman > 1){
	echo  "<a href='produk-ukm.html?ukm=".$idUKM."&halaman=".($nohalaman-1)."' class='nextprev'>&lt;&lt; Prev</a>";
}
...
for($halaman = 1; $halaman <= $jumhalaman; $halaman++)
{
		 ...
		 {   
			...
			if ($halaman == $nohalaman){ 
				...
				echo " <a href='produk-ukm.html?ukm=".$idUKM."&halaman=".$halaman." '>".$halaman."</a> ";
			}
			$showhalaman = $halaman;          
		 }
}
...
if ($nohalaman < $jumhalaman){ 
	echo "<a href='produk-ukm.html?ukm=".$idUKM."&halaman=".($nohalaman+1)."' class='nextprev'>Next &gt;&gt;</a>";
}
?>
